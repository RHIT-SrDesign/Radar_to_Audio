...
if __name__=='__main__':
    # example usage
    file_path = 'angry_deer_sas.tmp'
    print('Running tests on '+file_path+'...')
    mf = MidasFile(file_path)
    n_elements = 6e6 # read this many elements 
    n_overlap = int(n_elements) # overlap by a non-integer multiple just to check that things appear to be working
    n_fft = 1024
    window = sig.chebwin(n_fft,1000,sym=False) # any window will probably do
    
    # try reading an arbitrary window
    d = mf.read_at_time(0,1)
    print(d.shape)
    ...
    #f,t,s = sig.stft(np.real(d),mf.sample_rate,window='hann')
    s_disp = 20*np.log10(np.abs(s)/np.max(np.abs(s))) # make the spectrum easier to read
